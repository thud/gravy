...
import { sleepPromise, resetAnimations, playVisualisation } from './Utils.js'

export function calcDFS(ns, cns, startid, endid, leave_open = false) {
    const visited_nodes = new Map()
    const visited_cns = new Map()
    const flipped_cn = new Map()
    const node_part_of_final_path = new Set()
    const cn_part_of_final_path = new Set()
    let time_counter = 0

    function runDFS(i) {
        const nodea = ns.find((node) => node.id === i)
        ...
        for (const cnid of nodea.direct_cn) {
            const cn = cns.find((conn) => conn.id === cnid)
            if (!cn) continue
    ...
    visited_nodes.forEach((offset, nid) => {
        const nextnode = ns.find((node) => node.id === nid)
    ...
    visited_cns.forEach(([startoffset, endoffset], cnid) => {
        const nextcn = cns.find((cn) => cn.id === cnid)
...
export function calcBFS(ns, cns, startid, endid, leave_open = false) {
    const visited_nodes = new Map()
    const visited_cns = new Map()
    const flipped_cn = new Map()
    const q = [[startid, -1]]
    ...
    while (q.length) {
        ...
        const nodea = ns.find((node) => node.id === i)
        if (!nodea) console.error('no node found with id ', i)

        if (cns.find((cn) => cn.id === from_cnid)) {
    ...
    if (found) {
        let c_id = endid
        while (c_id !== startid) {
            const c_node = ns.find((node) => node.id === c_id)
            if (!c_node) console.error('no node found with id ', c_id)
            ...
            const c_cn = cns.find((cn) => cn.id === c_cnid)
            if (!c_cn) console.error('no cn found with id ', c_id)
        ...
        const start_node = ns.find((node) => node.id === startid)
...
export function calcDijkstra(ns, cns, startid, endid, leave_open = false) {
    const visited_nodes = new Map()
    const visited_cns = new Map()
    const flipped_cn = new Map()
    let q = [[startid, -1, 0]]
...
export function calcBiDijkstra(ns, cns, startid, endid) {
    const visited_nodes = new Map()
    ...
    if (endid === startid) return calcDijkstra(ns, cns, startid, endid, true)
    let q = [
        [startid, -1, 0, true],
        [endid, -1, 0, false],
    ]
    ...
    if (found) {
        ...
        const meet_node_second_cn = cns.find(
            (cn) => cn.id === meet_node_second_cnid
        )
        if (meet_node_second_cn) {
            ...
            const second_node = ns.find((node) => node.id === second_node_id)
            if (second_node) {
                c_id = second_node_id
                do {
                    const c_node = ns.find((node) => node.id === c_id)
        ...
        const end_node = ns.find((node) => node.id === endid)
        if (end_node) {
...
export function calcAStar(ns, cns, startid, endid, leave_open = false) {
    const hcostweight = 0.1
    const visited_nodes = new Map()
    const visited_cns = new Map()
    const flipped_cn = new Map()
    const endnode = ns.find((node) => node.id === endid)
    let endnode_pos
    ...
    while (q.length) {
        ...
        nodea.direct_cn.forEach((cnid) => {
            ...
            if (cn) {
                ...
                if (!visited_nodes.has(nodebid)) {
                    visited_nodes.set(nodebid, [i, cnid, -1])
                    const nodeb = ns.find((node) => node.id === nodebid)
                    if (!nodeb) console.error('no node found with id ', nodebid)
...
export function calcTopDFS(ns, cns, startid, _, __, ___) {
    const visited_nodes = new Map()
    const visited_cns = new Map()
    const flipped_cn = new Map()
    ...
    function runDFS(i, tmp_seen = new Set()) {
        const nodea = ns.find((node) => node.id === i)
        if (!nodea) console.error('no node found with this id')
        visited_nodes.set(i, ++time_counter)
        tmp_seen.add(i)
    ...
    let okgraph = true
    okgraph &= runDFS(startid)
    while (visited_nodes.size != ns.length && okgraph) {
        for (const { id } of ns) {
    ...
    stack.forEach(([nid, from_cnid, offset]) => {
        ...
        if (from_cnid !== -1) {
            const from_cn = cns.find((cn) => cn.id === from_cnid)
    ...
    cns.forEach((cn) => {
        const ae = get(cn.animationEvents)
        if (!cns_changed_in_sort.has(cn.id) && !cns_changed_in_dfs.has(cn.id)) {
            ae.push(
                {
                    method: 'set_processingState',
                    newState: 0,
...
export function calcTopKhan(ns, cns, startid, _, __, ___) {
    const visited_nodes = new Map()
    const visited_cns = new Map()
    const flipped_cn = new Map()
    const stack = []
    ...
    ns.forEach((node) => {
        incoming.set(node.id, node.indirect_cn.size)
...
export function calcKosaraju(ns, cns, startid, _, __, ___) {
    const visited_nodes1 = new Map()
    const visited_nodes2 = new Map()
    const visited_cns1 = new Map()
    const visited_cns2 = new Map()
    const flipped_cn1 = new Map()
    const flipped_cn2 = new Map()
    const stack = []
    const node_scc = new Map()
    const cn_scc = new Map()
    let time_counter = -1
    ...
    runDFS1(startid)
    while (visited_nodes1.size != ns.length) {
        for (const { id } of ns) {
    ...
    function runDFS2(i, nodesinscc, cn_already_part_of_scc, root = i) {
        const nodea = ns.find((node) => node.id === i)
        if (!nodea) console.error('no node found with this id')
    ...
    do {
        ...
        const nodesinscc = new Set()
        const cn_already_part_of_scc = new Set()
...
export function calcTarjan(ns, cns, startid, _, __, ___) {
    const visited_nodes = new Map()
    const visited_cns = new Map()
    const flipped_cn = new Map()
    const stack = []
    ...
    runDFS(startid)
    while (visited_nodes.size != ns.length) {
        for (const { id } of ns) {
            if (!visited_nodes.has(id)) {
                stack.push([id, -1])
                runDFS(id)
...
export function calcHamiltonian(ns, cns, startid, _, __) {
    const visited_nodes = new Map()
    const visited_cns = new Map()
    const node_part_of_final_path = new Set()
    const cn_part_of_final_path = new Set()
    ...
    function runDFS(i) {
        ...
        if (i === startid && node_part_of_final_path.size)
            return node_part_of_final_path.size === ns.length
        for (const cnid of nodea.direct_cn) {
            const cn = cns.find((conn) => conn.id === cnid)
            if (!cn) continue
            ...
            if (
                ...
                if (
                    nodebid !== startid ||
                    node_part_of_final_path.size === ns.length
                )
...
export function calcTS(ns, cns, startid, _, __) {
    const visited_nodes = new Map()
    const visited_cns = new Map()
    ...
    function runDFS(i, weightsum = 0) {
        ...
        if (i === startid && node_part_of_path.size) {
            if (node_part_of_path.size === ns.length) {
        ...
        for (const cnid of nodea.direct_cn) {
            ...
            if (!node_part_of_path.has(nodebid) && !cn_part_of_path.has(cnid)) {
                ...
                if (nodebid !== startid || node_part_of_path.size === ns.length)
