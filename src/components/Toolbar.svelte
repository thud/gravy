 <script>
	import { colors, toolbar_visible, show_cn_weights, show_cn_directions, default_cn_weight, create_start_node, create_end_node, start_node_id, end_node_id, orb_number, orb_speed, mode } from "../model/State.js";
	...
	function handleClickView(m) {
		switch (m) {
			case 0:
				return () => {$show_cn_weights = !$show_cn_weights;};
			case 1:
				return () => {$show_cn_directions = !$show_cn_directions;};
		}
	}
...
<div 
	...
	style="background-color:{$colors.nord5}; height:calc(60px + {$heightTween}%); top: calc(95% - {$heightTween}% - 60px);overflow:{$toolbar_visible ? 'scroll' : 'hidden'}"
	...
	<div class="toolbar-section">
		<div class="toolbar-section-label">
			Special
		</div>
		<div class="toolbar-row">
			<div
				on:click={_ => {$create_start_node=true;}}
			...
			<div
				on:click={_ => {$create_end_node=true;}}
				class="toolbar-row-element"
				id="toolbar-create-start-node"
				style="color:{$colors.nord0}">
				Create End Node
	...
	<div class="toolbar-section">
		...
		<div class="toolbar-row nohover">
			...
			<input
				...
				style="color:{$colors.nord0};background-color:{valid_default_cn_weight ? $colors.nord5 : $colors.nord13}; width: 70px; flex: 0;"
		...
		<div class="toolbar-row">
			...
			<input
				...
				type="range"
				min="1" max="15" step="1"
				on:change={_ => {$show_cn_directions = true;}}
		...
		<div class="toolbar-row">
			...
			<input
				...
				style="color:{$colors.nord0};background-color:{valid_default_cn_weight ? $colors.nord5 : $colors.nord13};width:70px;"
				type="range"
				min="0.00001" max="0.0005" step="0.000001"
				on:change={_ => {$show_cn_directions = true;}}
